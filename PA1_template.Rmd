...
ggplot(activity, aes(x=date)) +
        geom_histogram(aes(weight = steps), binwidth = 0.1) +
...
activitymean
```
We can see that the mean number of steps taken each day is under each date.

```{r}
activitymedian <- tapply(activity$steps, activity$date, FUN = median, na.rm=TRUE)
activitymedian
```
We can see that the median number of steps taken each day is under each date.
...
```{r}
ggplot(activityfill, aes(x=date)) +
        geom_histogram(aes(weight = steps), binwidth = 0.1) +
        ylab("Total number of steps") +
        xlab("Date") +
        ggtitle("The total number of steps taken each day (missing data are filled)")
```

Calculate the mean and median number of steps taken each day based on filled data.
```{r}
activityfillmean <- tapply(activityfill$steps, activityfill$date, FUN = mean, na.rm=TRUE)
activityfillmedian <- tapply(activityfill$steps, activityfill$date, FUN = median, na.rm=TRUE)
```
